...
export default function ChatWindow({ messages, currentResponse, isLoading, error }: ChatWindowProps) {
      ...
      {isLoading && currentResponse && (
        <div className="flex justify-start">
          <div className="max-w-[85%] sm:max-w-[75%] rounded-lg p-3 whitespace-pre-wrap break-words bg-theme-panel-bg border border-theme-border-primary">
            {currentResponse}
            <span className="inline-block w-2 h-4 ml-1 bg-theme-text-primary animate-pulse">|</span>
          </div>
        </div>
      )}
      {error && (
