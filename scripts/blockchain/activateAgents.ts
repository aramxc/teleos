...
// Get private key from env and add '0x' prefix if needed
...
async function activateAllAgents(agentIds: string[]) {
    try {
        ...
        // Split into chunks of 50 to avoid gas limits
        const chunkSize = 50;
        for (let i = 0; i < agentIds.length; i += chunkSize) {
            const chunk = agentIds.slice(i, i + chunkSize);
            
            console.log(`Activating agents ${i + 1} to ${i + chunk.length}...`);
            const tx = await contract.bulkSetAgentStatus(chunk, true);
            await tx.wait();
...
// Your actual agent IDs from the database
const agentIds = [
